...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {// 调用before方法实现用户自定义的bean实例化方法
					//InstantiationAwareBeanPostProcessor接口回调，
					//一旦用户自定义实现了该方法返回了具体的对象而不是null，
					// 那么意味着Spring自身的bean创建流程到此被阻断，将提前返回用户自主创建的bean对象给上层。
					//前置处理器
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					if (bean != null) {
						//BeanPostProcessor接口回调，后置处理器
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		//获取bean定义中的属性集合，这里面一般封装了 <property/>的值
	...
	protected void autowireByName(
		 ...
		 *
		 * 除了要求非简单类型的属性外，还要求属性未在配置文件中配置过，即
		 * <bean id="iocByNameTest" class="org.springframework.context.mytest.IocByNameTest" autowire="byName">
		  ...
		  当autowire="byName"，property 标签为空才能进行 byName自动注入，否则仍采用setter方法注入，
		 * ，也就是 pvs.contains(pd.getName()) = false
		 * 且有set方法
		 *
		 */
		...
		for (String propertyName : propertyNames) {
			...
			if (containsBean(propertyName)) {//根据属性名称匹配
				//实例化依赖，循环依赖响应点
				Object bean = getBean(propertyName);
				//属性名 - 属性值 保存在 MutablePropertyValues pvs，后续统一通过反射设置到bean中
			...
			else {//日志，通过by Name 方式，没找到匹配的bean
				if (logger.isTraceEnabled()) {
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		...
		//property属性标签 如：<property name="name" value="setter方法"></property>
		PropertyValues pvs = mbd.getPropertyValues();
		//获得类的全部属性列表
		PropertyDescriptor[] pds = bw.getPropertyDescriptors();
		for (PropertyDescriptor pd : pds) {
			//1.pd.getWriteMethod()要有set方法
			// 2.!pvs.contains(pd.getName())--> 属性 ，xml中不能设置该属性的property标签，不能主动赋值
			// 3.非简单类型
			// 4.排除ignored dependency type和 defined by CGLIB
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		for (PropertyValue pv : original) {    //遍历所有属性
			if (pv.isConverted()) {
				...
				//解析 value,针对 setter 方式注入的依赖进行解析
		...
		// Set our (possibly massaged) deep copy. 利用反射获取set方法进行设置
		try {

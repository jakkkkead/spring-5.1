...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (containingBean == null) {
			//校验beanName的唯一性，在同一个Beans标签下面不能重复
			checkNameUniqueness(beanName, aliases, ele);
		}
		// 根据 <bean ...>...</bean> 中的配置创建 BeanDefinition，然后把配置中的信息都设置到实例中,
		// 这行执行完毕，一个 BeanDefinition 实例就出来了
		AbstractBeanDefinition beanDefinition = parseBeanDefinitionElement(ele, beanName, containingBean);
		if (beanDefinition != null) {//<bean/>标签设置完成
			if (!StringUtils.hasText(beanName)) {
				//如果没有指定beanName使用，使用spring规则生成beanName
				try {
					if (containingBean != null) {
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			//创建AbstractBeanDefinition，设置类信息
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		for (int i = 0; i < nl.getLength(); i++) {//获取property的子标签
			Node node = nl.item(i);
		...
		//在property标签中，value和ref 属性只允许存在一种
		...
		else if (subElement != null) {//property下的子标签
	...
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		//获取命名空间，如：xmlns:tx="http://www.springframework.org/schema/tx"
